...
Because GitHub suppresses ordinary workflow recursion for PRs created with `GITHUB_TOKEN`, the trusted writer explicitly dispatches the four required checks on the bot branch. That dispatch is accepted only on the exact branch, only for files declared by the generated-files contract, and only when rerunning `sync:repo-state` produces the exact full Git tree. A trusted waiter binds the open PR to its immutable head, verifies all four exact GitHub Actions checks, confirms that `main` is unchanged, performs an immediate exact-head squash merge, and explicitly dispatches main CI, Pages, and CodeQL. When GitHub reports `main` as protected, that is recorded as an additional signal; when it cannot be proven (for example on a personal repository without branch protection configured), the merge still fails closed unless the exact base SHA is unchanged and every required check succeeded on the exact head. The detailed protection policy is configured and audited with maintainer credentials; the workflow token has no bypass around it.
